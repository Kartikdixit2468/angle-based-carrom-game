...
function resizeCanvas() {
    const paddingX = 20; // Side margins for mobile
    const paddingY = 160; // Space for UI on top and bottom
    const minDimension = Math.min(window.innerWidth - paddingX, window.innerHeight - paddingY);
    boardSize = Math.max(300, Math.min(600, minDimension)); // Limit max size to 600px so it's not huge on desktop
    canvas.width = boardSize;
    canvas.height = boardSize;

    // Make border 3x broader, scaling with the board size
    canvas.style.borderWidth = `${Math.max(24, boardSize * 0.08)}px`;
    ...
    pockets = [
        ...
        { x: boardSize - pocketOffset, y: boardSize - pocketOffset, r: pocketRadius }
    ];
}
...
function drawTrajectoryAndAngles(ctx) {
    ...
    if (dirX < 0) {
        ...
        if (t > 0 && t < tMin) { tMin = t; hitNormalX = 1; hitNormalY = 0; }
    } else if (dirX > 0) {
    ...
    if (dirY < 0) {
        ...
        if (t > 0 && t < tMin) { tMin = t; hitNormalX = 0; hitNormalY = 1; }
    } else if (dirY > 0) {
    ...
    if (visualPowerLimit === tMin) { // We hit a wall visually
        ...
        drawAngleArc(ctx, endX, endY, arcRadius, angleNormal, angleIn, 'rgba(239, 68, 68, 0.8)');
        // Draw Reflection Arc (Blue)
        drawAngleArc(ctx, endX, endY, arcRadius, angleNormal, angleOut, 'rgba(14, 165, 233, 0.8)');
        
        // Calculate degree value
        const dotIn = (-dirX * hitNormalX) + (-dirY * hitNormalY);
        let degreeVal = Math.round(Math.acos(Math.max(-1, Math.min(1, dotIn))) * 180 / Math.PI);
        ...
        const labelDist = 55; 
        ctx.fillText(`${degreeVal}Â°`, endX + hitNormalX * labelDist, endY + hitNormalY * labelDist);
...
function drawBoard() {
    ...
    function drawCapsuleLane(cx, cy, isHoriz) {
        ...
        const yellowR = creamR * 0.70; // Increased radius of yellow circle
